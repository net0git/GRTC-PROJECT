  ...
  ObtenerDatosPersonal(id:number){
    this.personaService.ObtenerPersona(id).subscribe(
      res=>{
          ...
          const tipoDocSelect = document.getElementById('tipo_doc') as HTMLSelectElement;
          // Establece el valor seleccionado deseado
          const valorSeleccionado = this.data_persona.tipo_doc; // Puedes establecer el valor que desees
          tipoDocSelect.value = valorSeleccionado;
